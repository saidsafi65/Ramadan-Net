...
<body>
  <div class="container">
    ...
    <div class="content">
      <form method="POST" action="{{ route('registrat') }}">
        @csrf
        <div class="user-details">
          <div class="input-box">
            <span class="details">الاسم بالكامل</span>
            <input class="form-control" type="text" id="name" name="name" placeholder="ادخل الاسم باللغة العربية" required>
            @error('name')<span class="text-danger">{{ $message }}</span>@enderror
          </div>
          <div class="input-box">
            <span class="details">الاسم المستخدم باللغة الانجليزية</span>
            <input class="form-control" type="text" id="username" name="username" placeholder="ادخل اسم الحساب باللغة الانجليزية فقط" onkeypress="return (event.charCode >= 65 && event.charCode <= 90) || (event.charCode >= 97 && event.charCode <= 122) || (event.charCode >= 48 && event.charCode <= 57)" required>
            @error('username')<span class="text-danger">{{ $message }}</span>@enderror
          </div>
          <div class="input-box">
            <span class="details">البريد الالكتروني</span>
            <input class="form-control" type="email" id="email" name="email" placeholder="ادخل البريد الالكتروني الخاص بك" required>
            @error('email')<span class="text-danger">{{ $message }}</span>@enderror
          </div>
          <div class="input-box">
            <span class="details">رقم الجوال</span>
            <input class="form-control" type="number" maxlength="10" id="mobile" name="mobile" placeholder="ادخل رقم الجوال" required>
            @error('mobile')<span class="text-danger">{{ $message }}</span>@enderror
          </div>
          <div class="input-box">
            <span class="details">كلمة المرور</span>
            <input class="form-control" type="password" id="password" name="password" placeholder="كلمة المرور" required>
            @error('password')<span class="text-danger">{{ $message }}</span>@enderror
          </div>
          <div class="input-box">
            <span class="details">تأكيد كلمة المرور</span>
            <input class="form-control" type="password" id="password_confirmation" name="password_confirmation" placeholder="تأكيد كلمة المرور" required>
